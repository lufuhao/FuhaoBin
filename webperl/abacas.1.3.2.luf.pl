...
use Getopt::Std;
our $version="1.3.2 revised by luf";
#-------------------------------------------------------------------------------

help() if (@ARGV < 1);

my ($help, $reference, $query_file, $choice, $sen, $seed, $mlfas, $fasta_bin, $avoid_Ns, $tbx, $min_id, $min_cov, $diff_cov, $min_len,$add_bin_2ps, $pick_primer, $flank, $chk_uniq,$redoMummer, $is_circular,$escapeToPrimers, $debug, $gaps_2file, $prefix,$optionsLog,$numNs)=checkUserInput( @ARGV );
...
sub checkUserInput{
	...
	my ($help, $reference, $query_file, $choice, $sen, $seed, $mlfas, $fasta_bin, $avoid_Ns, $tbx, $min_id, $min_cov, $diff_cov, $min_len,$add_bin_2ps, $pick_primer, $flank, $chk_uniq,$redoMummer, $is_circular,$escapeToPrimers, $debug, $gaps_2file, $prefix);
...
# print $redoMummer , "\n"; exit;
	...
	#print $optionsLog;
	return ($help, $reference, $query_file, $choice, $sen, $seed, $mlfas, $fasta_bin, $avoid_Ns, $tbx, $min_id, $min_cov, $diff_cov, $min_len,$add_bin_2ps, $pick_primer, $flank, $chk_uniq,$redoMummer, $is_circular,$escapeToPrimers, $debug, $gaps_2file, $prefix,$optionsLog,$numNs);
...
sub Fasta2ordered {
	...
	my @fasta = split (/\n/, $fast);
	#remove chromosome name if exists in the onput sequence.
	if ($fasta[0] =~ /\>/ ) {
		my $ch_name = $fasta[0];
		shift @fasta;
	}
...
### has as index the postion with the max hits
	...
	foreach (@File) {
		...
		if ($is_promer ==1) {
			...
			elsif ($key ne $a[12]) {### here possible bugg...
				$h_max{$tmp_index}=$key;
				$key=$a[12];
				$tmp=$a[5];       # length
				$tmp_index=$a[0]; # position reference
			...
			elsif ($key ne $a[8]) {### here possible bugg...
				$h_max{$tmp_index}=$key;
				$key=$a[8];
				$tmp=$a[5];       # length
				$tmp_index=$a[0]; # position reference
